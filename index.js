...
import { getUserPlanBySlug, checkDeviceBySlug, ensureDeviceBySlug, db, users, getSubscriptionStatus, updateSubscription, getDatabaseUserByEmail, createDatabaseUser, upgradeDatabaseUser } from "./RK_AI_HOME/services/appwriteClient.js";
...
import { handleIntents } from "./RK_AI_HOME/taskHandler.js";
import { cleanupSupabaseFiles, migrateToGoogleDrive, listFilesFromSlug, downloadFileFromSlug, deleteFileFromSlug } from "./RK_AI_HOME/services/supabaseClient.js";
import desktopRouter from "./RK_AI_DESKTOP/index.js";
import billingRouter from "./RK_AI_DESKTOP/endpoints/billing.js";
...
async function validateDevice(req, res, next) {
  ...
  try {
    ...
    logError("Device validation error:", err);
    return res.status(500).json({ error: "Server error during device validation" });
  }
}
...
// ---------------- ALL AUTH ENDPOINTS (UNPROTECTED) ----------------
// ---------------- GOOGLE OAUTH START ----------------
...
app.get("/auth/google/callback", async (req, res) => {
  try {
    ...
    if (isWebFlow) {
      ...
      // Ensure user exists in custom users database collection for slug and plan
      let dbUser = await getDatabaseUserByEmail(userInfo.email);
      if (!dbUser) {
          dbUser = await createDatabaseUser(userInfo.email, userInfo.name || userInfo.email.split("@")[0]);
      }

      // Redirect to frontend with token, userId, slug, and plan
      ...
      return res.redirect(`${FRONTEND_URL}/auth/web-callback?token=${appwriteUser.$id}&userId=${appwriteUser.$id}&slug=${dbUser.slug}&plan=${dbUser.plan}&redirect=${encodeURIComponent(webRedirect)}`);
    }
    ...
    return res.redirect(`${baseRedirectUrl}?google_connected=true&email=${encodeURIComponent(userInfo.email || '')}&username=${encodeURIComponent(userInfo.name || '')}`);
  } catch (err) {
...
app.use("/rk-ai-desktop", desktopRouter);
...
app.post("/desktop/login", async (req, res) => {
  try {
    ...
    // Link Web Auth Session to Custom Users Database Collection
    let dbUser = await getDatabaseUserByEmail(email);
    if (!dbUser) {
        dbUser = await createDatabaseUser(email, data.name || email.split("@")[0]);
    }
    
    // Override device_slug and plan for the Desktop App to use
    data.slug = dbUser.slug;
    data.plan = dbUser.plan;
...
app.get("/desktop/profile/:userId", async (req, res) => {
  const { userId } = req.params;
  try {
    const subscriptionsReq = await db.listDocuments(
      process.env.APPWRITE_DB_ID,
      "subscriptions",
    ...
    console.error("[Desktop Profile Error] failed to check:", err);
...
app.get("/device/:slug/profile", async (req, res) => {
  try {
    const slug = normalizeSlug(req.params.slug);
    const device = await getUserPlanBySlug(slug);
    if (!device) return res.status(404).json({ error: "Device not found" });
    return res.json({ email: device.email || "", username: device.name || "" });
  } catch (err) {
    return res.status(500).json({ error: "server_error" });
  }
});
...
// --- QUANTUM RELAY (E2E Encrypted Sync) ---
const quantumRelayStore = new Map();

app.post("/relay/sync", async (req, res) => {
  const slug = req.headers["x-device-slug"] || req.body.slug;
  if (!slug) return res.status(400).json({ error: "Device slug required" });
  
  const normalizedSlug = normalizeSlug(slug);
  const { encrypted_blob } = req.body;
  
  if (!encrypted_blob) return res.status(400).json({ error: "No encrypted payload provided" });
  
  // Store the encrypted state blob. The server CANNOT read this payload.
  quantumRelayStore.set(normalizedSlug, {
    blob: encrypted_blob,
    timestamp: Date.now()
  });
  
  console.log(`[Quantum Relay] Encrypted state synced for slug: ${normalizedSlug}`);
  res.json({ ok: true, message: "Encrypted state securely relayed" });
});

app.get("/relay/sync/:slug", async (req, res) => {
  const normalizedSlug = normalizeSlug(req.params.slug);
  const state = quantumRelayStore.get(normalizedSlug);
  
  if (state) {
    res.json({ ok: true, encrypted_blob: state.blob, timestamp: state.timestamp });
  } else {
    res.status(404).json({ error: "No relay state found for this device" });
  }
});
// ---------------- START SERVER ----------------
